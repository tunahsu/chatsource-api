...
refresh_security = JwtRefreshBearer(secret_key=settings.SECRET_KEY,
...
def refresh_tokens(
    ...
    # Customize expires_delta when creating
    access_token = access_security.create_access_token(
        subject=credentials.subject)
    refresh_token = refresh_security.create_refresh_token(
        subject=credentials.subject, expires_delta=timedelta(days=30))
    return {
